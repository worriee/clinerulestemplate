...
- **Prohibited Background Mutating**: Do not alter, overwrite, or touch `.roo/project_memory.md` or any files inside `.roo/memory/` during feature development or bug patches unless the user explicitly included the specific trigger flags (`-context`, `-error`, `-codebase`, `-setup`, `-archive`).
