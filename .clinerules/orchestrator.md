...
2. **Delegation**: Explicitly guide the flow of execution based on single-letter flags or workflow state:
   - Direct to `planner.md` (flag: `-p`) for architectural mapping and specification gathering.
   - Direct to `coder.md` (flag: `-c`) for clean feature implementation.
   - Direct to `debugger.md` (flag: `-d`) for addressing unit test failures or integration bugs.
   - Direct to `ask.md` (flag: `-a`) for querying complex documentation or existing patterns.
   - Direct to `secure.md` (flag: `-s`) for running exploit modeling, computing security ratings, and identifying architectural vulnerabilities.
